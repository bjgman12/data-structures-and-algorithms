...
const filterStringsWithVowels = (arr) => {
  // Solution code here...
  let vowelsOnly = arr.filter(val => {
    return (/[aeiou]/).test(val);
  });
  return vowelsOnly;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  ...
  return arr.filter((val) => {
    return !(val === forbiddenValues[forbiddenValues.indexOf(val)]);
  });
};
...
const evenOddNumericValues = (arr) => {
  // Solution code here...
  let numberfilt = arr.filter(val => {
    return (typeof (val) === 'number');
  });

  return numberfilt.map(val => {
    if(val % 2 === 0) {
      return 'even';
    } else {
      return 'odd';
