...
const createServer = () => {
  ...
  app.get('/aboutme', (req, res) => {
    res.send('blalallasdsdasdal');
  });
  app.get('/favoritefoods', (req, res) => {
    let foods = ['apple', 'bannanas', 'carrots'];
    res.send(foods);
  });
  app.get('*', (req, res) => {
...
const count = (target, input) => {
  ...
  let newArr = input.map(num => {
    let new2d = num.map(val => {
      if (val === target) {
        count++;
      }
    });
...
const totalSum = (input) => {
  // Solution code here...
  let placeholder = input.map(row => {
    let test = row.reduce((newValue, currentVal) => {
      return newValue + currentVal;
    }, 0);
    return test;
  });
  let retVal = placeholder.reduce((newValue, currentVal) => {
    return newValue + currentVal;
  }, 0);
  return retVal;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let new2d = [];
  input.forEach( row => {
    let test = row.reduce( (newValue,currentValue) => {
