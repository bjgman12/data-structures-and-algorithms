...
const extractAbilities = (arr) => {
  let abilitiesOnly = arr.map(ability => {
    return ability.ability.name;
  }
  );
  return abilitiesOnly;
};
...
const extractStats = (arr) => {
  function Stat(obj) {
    ...
    this.total = obj.effort + obj.baseStat;
  }
  let statArr = arr.map(stat => {
    let formatedStat = new Stat(stat);
    return formatedStat;
  });
  console.log('This is literally what the prompt is asking for.');
  return statArr;
};
